...
const Home = () => {
  ...
  if (products && (stocks || brand.length)) {
    content = products.filter(pd => {
      ...
      .filter(pd => {
        if (brand.length) {
          return brand.includes(pd.brand)
        }
        return pd
      }).map((product) => (
        <ProductCard key={product.model} product={product} />
  ...
  return (
    <div className='max-w-7xl gap-14 mx-auto my-10'>
      <div className='mb-10 flex justify-end gap-5'>
        <button onClick={() => dispatch(toggleStock())} className={`border px-3 py-2 rounded-full font-semibold ${stocks ? activeClass : null} `}
        >
          In Stock
        </button>
